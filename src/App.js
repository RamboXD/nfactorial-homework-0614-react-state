...
function App() {
  ...
  const localStorageToDoItems = JSON.parse(localStorage.getItem("listToDo"))
    ? JSON.parse(localStorage.getItem("listToDo"))
    : [];

  const [items, setItems] = useState(
    localStorageToDoItems.length > 0 ? localStorageToDoItems : toDoItems
  );
